...
//
// 	for i, _ := range r {
// 		logger.Info("to encode", "type", outputParams[i])
// 		switch outputParams[i] {
// 		case "int":
// 			logger.Info("int encode")
// 			err = encd.Encode(r[i].Int())
// 			panik.On(err)
// 		case "map":
// 			logger.Info("map encode")
// 			err = encd.Encode(r[i].Interface().(map[string]interface{}))
// 			panik.On(err)
// 		case "string":
// 			logger.Info("string encode")
// 			err = encd.Encode(r[i].String())
// 			panik.On(err)
// 		case "*st:github.com/thejackrabbit/aqua.Sac":
// 			logger.Info("*sac encode")
// 			err = encd.Encode(r[i].Elem().Interface().(Sac).Data)
// 		case "i:.":
// 			v := r[i].Elem()
// 			it := getSignOfType(v.Type())
// 			// log.Info("value", getSignOfObject(v))
// 			// log.Info("value.Elem()", getSignOfObject(v.Elem()))
// 			// log.Info("type", getSignOfType(v.Type()))
// 			// log.Info("type.Elem()", getSignOfType(v.Elem().Type()))
//
// 			switch {
// 			case it == "map":
// 				err = encd.Encode("map")
...
// 				err = encd.Encode(r[i].Interface().(map[string]interface{}))
// 				panik.On(err)
// 			case strings.HasPrefix(it, "st:"):
// 				logger.Info("st encode", "type", it)
// 				err = encd.Encode("st")
// 				panik.On(err)
// 				err = encd.Encode(v)
// 				panik.On(err)
// 			case strings.HasPrefix(it, "sl:"):
// 				logger.Info("sl encode", "type", it)
// 				err = encd.Encode("sl")
// 				panik.On(err)
// 				err = encd.Encode(v)
// 				panik.On(err)
// 			default:
// 				panic("Unmanaged interface type sent to caching: " + outputParams[i] + " : " + it)
// 			}
// 		default:
...
//
// 	logger.Info("Fn decomposeCachedValues()")
//
// 	var err error
...
// 		switch o {
// 		case "int":
// 			logger.Info("int decode")
// 			var j int
// 			err = decd.Decode(&j)
// 			panik.On(err)
// 			out[i] = reflect.ValueOf(j)
// 		case "map":
// 			logger.Info("map decode")
// 			var m map[string]interface{}
// 			err = decd.Decode(&m)
// 			panik.On(err)
// 			out[i] = reflect.ValueOf(m)
// 		case "string":
// 			logger.Info("string decode")
// 			var s string
// 			err = decd.Decode(&s)
// 			panik.On(err)
// 			out[i] = reflect.ValueOf(s)
// 		case "*st:github.com/thejackrabbit/aqua.Sac":
// 			logger.Info("*sac decode")
// 			var m map[string]interface{}
// 			err = decd.Decode(&m)
// 			panik.On(err)
// 			s := NewSac()
...
// 			switch s {
// 			case "st":
// 				logger.Info("st decode")
// 				var m map[string]interface{}
// 				err = decd.Decode(&m)
// 				panik.On(err)
// 				out[i] = reflect.ValueOf(m)
// 			case "sl":
// 				logger.Info("sl decode")
// 				var a []interface{}
// 				err = decd.Decode(&a)
// 				panik.On(err)
// 				out[i] = reflect.ValueOf(a)
// 			default:
// 				panic("Unmanaged type sent for decoding in cache:" + o + " : " + s)
// 			}
// 		default:
